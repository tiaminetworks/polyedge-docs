...
The GUI provides:
- **Configuration** — set NR parameters (band, frequency, SCS, PRBs, cell ID), gNB/sensor location, and start/stop the radio and streamer
...
Everything the GUI does is backed by a REST API on the same port (3000), and the detection pipeline exposes its own separate REST/WebSocket/MQTT/TAK outputs. See:
