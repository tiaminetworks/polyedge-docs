...
PolyEdge deployments update themselves by fetching a version manifest and reinstalling the four packages. No manual `.deb` download is required once configured.
...
2. That endpoint runs `/opt/polyedge/bin/polyedge-update.sh`, which:
   ...
   - Installs the packages via `apt`, in dependency order (`polyedge-runtime` then `polyedge-streamer` then `polyedge-interact`, plus any others listed).
   - Updates Python dependencies in the ISAC Engine's virtual environment.
...
This restarts the GUI service as part of the update. Any active browser session or API client connection will drop momentarily; this is expected, not an error. Progress is logged to `/var/log/polyedge-update.log` and streamed over the GUI's Socket.IO connection (`log_update` events) while it runs.

## Notes

- `polyedge-radio` is not part of the default update rotation, since it has an independent build chain. Update it separately if a new radio-stack version is needed.
