...
- **[Orchestrator API reference]({{ site.baseurl }}/api/orchestrator.html)** — radio control, configuration, process lifecycle (port 3000)
- **[Detection API reference]({{ site.baseurl }}/api/detection.html)** — passive radar detection output (port 8766)

## Authentication

The Orchestrator and Detection APIs share one optional credential: an `API_KEY` field embedded in your PolyEdge license. No key embedded, no authentication required — this is the default. Both reference pages above document the `X-API-Key` header where it applies.
